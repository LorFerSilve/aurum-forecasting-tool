"""Local HistData M1 ingestion for Phase-10 context research.

This adapter is intentionally separate from the frozen XAUUSD ingestion adapter.
It authenticates local annual archives and converts bid closes into provider-neutral
context observations. Historical availability remains a modeled-latency assumption.
"""

from __future__ import annotations

import hashlib
import io
import re
import zipfile
from datetime import UTC, datetime, timedelta, timezone
from pathlib import Path
from urllib.parse import urlsplit

import numpy as np
import pandas as pd
from pandas.errors import EmptyDataError, ParserError

from gold_forecasting.artifacts import sha256_file, write_json_atomic, write_parquet_atomic
from gold_forecasting.phase10.contracts import (
    DEVELOPMENT_END,
    DEVELOPMENT_START,
    OBSERVATION_COLUMNS,
    ContextSource,
    validate_observations,
)

HISTDATA_CONTEXT_SOURCE_OFFSET = "-05:00"
_MAX_ARCHIVE_BYTES = 256 * 1024 * 1024
_MAX_MEMBER_BYTES = 512 * 1024 * 1024
_SOURCE_TIMEZONE = timezone(
    -timedelta(hours=5),
    name=HISTDATA_CONTEXT_SOURCE_OFFSET,
)
_RAW_COLUMNS = (
    "source_timestamp",
    "bid_open",
    "bid_high",
    "bid_low",
    "bid_close",
    "volume",
)


class ContextHistDataError(ValueError):
    """Raised when local context source evidence cannot be trusted."""


def _as_utc(value: datetime | pd.Timestamp | None, *, name: str) -> pd.Timestamp:
    if value is None:
        return pd.Timestamp(datetime.now(UTC))
    result = pd.Timestamp(value)
    if result.tzinfo is None or str(result.tz_convert("UTC").tz) != "UTC":
        raise ContextHistDataError(f"{name} must be timezone-aware")
    return result.tz_convert("UTC").as_unit("ns")


def _validate_source(source: ContextSource, symbol: str) -> None:
    expected_source = {"XAGUSD": "silver", "EURUSD": "dollar"}.get(symbol)
    if expected_source is None or source.source_id != expected_source:
        raise ContextHistDataError("HistData context symbol/source_id binding is invalid")
    if source.availability_basis != "modeled_latency":
        raise ContextHistDataError(
            f"HistData {symbol} has no historical provider release timestamps; "
            "availability_basis must remain modeled_latency"
        )
    if source.revision_policy != "append_only":
        raise ContextHistDataError(f"HistData annual {symbol} archives use append_only semantics")
    parsed = urlsplit(source.source_url)
    if parsed.scheme != "https" or parsed.hostname not in {
        "histdata.com",
        "www.histdata.com",
    }:
        raise ContextHistDataError(
            f"HistData {symbol} source_url must use official HTTPS histdata.com"
        )


def _read_archive(path: Path) -> bytes:
    try:
        if path.is_symlink():
            raise ContextHistDataError("context archive must not be a symlink")
        if not path.is_file():
            raise ContextHistDataError(f"context archive is missing: {path}")
        size = path.stat().st_size
        if size <= 0 or size > _MAX_ARCHIVE_BYTES:
            raise ContextHistDataError("context archive has an invalid or excessive size")
        with path.open("rb") as stream:
            payload = stream.read(_MAX_ARCHIVE_BYTES + 1)
    except OSError as exc:
        raise ContextHistDataError(f"cannot read context archive: {path}") from exc
    if len(payload) > _MAX_ARCHIVE_BYTES:
        raise ContextHistDataError("context archive exceeds the configured size limit")
    return payload


def _members(zipped: zipfile.ZipFile, year: int, symbol: str) -> list[zipfile.ZipInfo]:
    pattern = re.compile(
        rf"(?:^|/)DAT_ASCII_{symbol}_M1_(\d{{4}})(?:\d{{2}})?\.csv$", re.IGNORECASE
    )
    matches: list[zipfile.ZipInfo] = []
    total_size = 0
    for info in zipped.infolist():
        if info.is_dir():
            continue
        # CRC checking reads ignored files too; bound the complete archive expansion.
        total_size += info.file_size
        if info.flag_bits & 0x1:
            raise ContextHistDataError(f"{symbol} archive contains an encrypted member")
        if total_size > _MAX_MEMBER_BYTES:
            raise ContextHistDataError(
                f"{symbol} archive expands beyond the safe member-size budget"
            )
        normalized = info.filename.replace("\\", "/")
        match = pattern.search(normalized)
        if match is None:
            continue
        if int(match.group(1)) != year:
            raise ContextHistDataError(
                f"{symbol} archive member period differs from the declared source year"
            )
        if info.file_size <= 0 or info.file_size > _MAX_MEMBER_BYTES:
            raise ContextHistDataError(f"{symbol} archive member has an invalid or excessive size")
        matches.append(info)
    if not matches:
        raise ContextHistDataError(
            f"ZIP archive contains no non-empty DAT_ASCII_{symbol}_M1_<period>.csv member"
        )
    return sorted(matches, key=lambda item: item.filename.casefold())


def _parse_member(
    zipped: zipfile.ZipFile,
    info: zipfile.ZipInfo,
    symbol: str,
) -> pd.DataFrame:
    try:
        with zipped.open(info, "r") as stream:
            raw = pd.read_csv(
                stream,
                sep=";",
                header=None,
                dtype="string",
                encoding="utf-8-sig",
                keep_default_na=False,
                na_filter=False,
            )
    except (EmptyDataError, ParserError, UnicodeError) as exc:
        raise ContextHistDataError(
            f"cannot parse Generic ASCII {symbol} member {info.filename!r}"
        ) from exc
    if raw.empty or raw.shape[1] != len(_RAW_COLUMNS):
        raise ContextHistDataError(
            f"{symbol} CSV must contain non-empty headerless "
            "timestamp;open;high;low;close;volume rows"
        )
    raw.columns = list(_RAW_COLUMNS)
    timestamp_text = raw["source_timestamp"].str.strip()
    if not timestamp_text.str.fullmatch(r"\d{8} \d{6}").all():
        raise ContextHistDataError(f"{symbol} source timestamp must use YYYYMMDD HHMMSS")
    try:
        local = pd.to_datetime(
            timestamp_text,
            format="%Y%m%d %H%M%S",
            exact=True,
            errors="raise",
        )
    except (TypeError, ValueError) as exc:
        raise ContextHistDataError(f"{symbol} archive contains an invalid timestamp") from exc
    if not local.dt.second.eq(0).all():
        raise ContextHistDataError(f"{symbol} archive contains a non-minute timestamp")

    result = pd.DataFrame(
        {
            "timestamp_open_utc": local.dt.tz_localize(_SOURCE_TIMEZONE).dt.tz_convert("UTC"),
            "source_member": info.filename,
        }
    )
    for column in ("bid_open", "bid_high", "bid_low", "bid_close", "volume"):
        try:
            values = pd.to_numeric(raw[column].str.strip(), errors="raise").astype("float64")
        except (TypeError, ValueError) as exc:
            raise ContextHistDataError(f"{symbol} {column} must be numeric") from exc
        if not np.isfinite(values.to_numpy()).all():
            raise ContextHistDataError(f"{symbol} {column} must be finite")
        result[column] = values

    prices = result[["bid_open", "bid_high", "bid_low", "bid_close"]]
    if prices.le(0).any().any():
        raise ContextHistDataError(f"{symbol} OHLC prices must be strictly positive")
    body_high = result[["bid_open", "bid_close"]].max(axis=1)
    body_low = result[["bid_open", "bid_close"]].min(axis=1)
    if (
        result["bid_high"].lt(body_high).any()
        or result["bid_low"].gt(body_low).any()
        or result["bid_high"].lt(result["bid_low"]).any()
    ):
        raise ContextHistDataError(f"{symbol} archive violates OHLC invariants")
    return result


def parse_histdata_context_archive(
    archive_path: str | Path,
    *,
    year: int,
    source: ContextSource,
    symbol: str,
    ingested_at_utc: datetime | pd.Timestamp | None = None,
) -> tuple[pd.DataFrame, dict[str, object]]:
    """Convert one authenticated annual archive into raw bid-close observations."""
    _validate_source(source, symbol)
    if type(year) is not int or year not in range(2020, 2025):
        raise ContextHistDataError(f"{symbol} development archive year must be 2020-2024")
    supplied_path = Path(archive_path).expanduser()
    payload = _read_archive(supplied_path)
    path = supplied_path.resolve()
    digest = hashlib.sha256(payload).hexdigest()
    ingested = _as_utc(ingested_at_utc, name="ingested_at_utc")

    frames: list[pd.DataFrame] = []
    try:
        with zipfile.ZipFile(io.BytesIO(payload)) as zipped:
            members = _members(zipped, year, symbol)
            bad_member = zipped.testzip()
            if bad_member is not None:
                raise ContextHistDataError(
                    f"{symbol} ZIP member failed its CRC check: {bad_member!r}"
                )
            frames = [_parse_member(zipped, info, symbol) for info in members]
    except zipfile.BadZipFile as exc:
        raise ContextHistDataError(f"{symbol} source file is not an intact ZIP archive") from exc

    candles = pd.concat(frames, ignore_index=True)
    local_year = candles["timestamp_open_utc"].dt.tz_convert(_SOURCE_TIMEZONE).dt.year
    if not local_year.eq(year).all():
        raise ContextHistDataError(f"{symbol} archive contains candles outside its source year")
    candles.sort_values(["timestamp_open_utc", "source_member"], kind="stable", inplace=True)
    exact = candles.duplicated(
        [
            "timestamp_open_utc",
            "bid_open",
            "bid_high",
            "bid_low",
            "bid_close",
            "volume",
        ],
        keep="first",
    )
    duplicates_removed = int(exact.sum())
    candles = candles.loc[~exact].copy()
    if candles["timestamp_open_utc"].duplicated().any():
        raise ContextHistDataError(
            f"{symbol} archive contains conflicting candles at one timestamp"
        )

    observed = candles["timestamp_open_utc"] + pd.Timedelta(minutes=1)
    development = observed.ge(DEVELOPMENT_START) & observed.lt(DEVELOPMENT_END)
    candles = candles.loc[development].copy()
    observed = observed.loc[development]
    if candles.empty:
        raise ContextHistDataError(f"{symbol} archive has no observations inside development scope")
    available = observed + pd.to_timedelta(source.publication_delay_seconds, unit="s")
    if ingested < available.max():
        raise ContextHistDataError(
            "ingested_at_utc precedes modeled availability for at least one observation"
        )

    identifier = observed.dt.strftime("%Y%m%dT%H%M%SZ")
    source_uri = (
        "https://www.histdata.com/download-free-forex-historical-data/"
        f"?/ascii/1-minute-bar-quotes/{symbol.lower()}/{year}"
    )
    observations = pd.DataFrame(
        {
            "source_id": source.source_id,
            "observation_id": (f"{symbol.lower()}-m1-" + identifier).to_numpy(),
            "observed_at_utc": observed.array,
            "available_at_utc": available.array,
            "ingested_at_utc": ingested,
            "value": candles["bid_close"].to_numpy(dtype=np.float64),
            "revision_id": f"sha256:{digest}",
            "source_uri": source_uri,
            "raw_sha256": digest,
        },
        columns=OBSERVATION_COLUMNS,
    )
    observations = validate_observations(observations, source)
    metadata: dict[str, object] = {
        "year": year,
        "path": str(path),
        "sha256": digest,
        "size_bytes": len(payload),
        "source_symbol": symbol,
        "source_timezone_offset": HISTDATA_CONTEXT_SOURCE_OFFSET,
        "source_observes_dst": False,
        "timestamp_semantics": "candle_open",
        "observation_semantics": "candle_close",
        "availability_basis": "modeled_latency",
        "publication_delay_seconds": source.publication_delay_seconds,
        "ingested_at_utc": ingested.isoformat(),
        "source_page_url": source_uri,
        "rows": len(observations),
        "exact_duplicates_removed": duplicates_removed,
        "members": sorted(candles["source_member"].unique().tolist()),
    }
    return observations, metadata


def _find_archive(directory: Path, year: int, symbol: str) -> Path:
    expected = f"HISTDATA_COM_ASCII_{symbol}_M1_{year}.zip".casefold()
    matches = [
        path
        for path in directory.iterdir()
        if path.is_file() and path.name.casefold() == expected
    ]
    if len(matches) != 1:
        raise ContextHistDataError(
            f"expected exactly one HISTDATA_COM_ASCII_{symbol}_M1_{year}.zip in {directory}"
        )
    return matches[0]


def import_histdata_context_archives(
    archive_directory: str | Path,
    output_directory: str | Path,
    source: ContextSource,
    *,
    symbol: str,
    years: tuple[int, ...] = (2020, 2021, 2022, 2023, 2024),
    ingested_at_utc: datetime | pd.Timestamp | None = None,
) -> dict[str, object]:
    """Build annual authenticated Parquet bundles and one bundle-set manifest."""
    _validate_source(source, symbol)
    if (
        not years
        or tuple(sorted(set(years))) != years
        or not set(years) <= set(range(2020, 2025))
    ):
        raise ContextHistDataError("years must be a sorted unique subset of 2020-2024")
    supplied_raw = Path(archive_directory).expanduser()
    if supplied_raw.is_symlink():
        raise ContextHistDataError("archive_directory must not be a symlink")
    raw = supplied_raw.resolve(strict=True)
    if not raw.is_dir():
        raise ContextHistDataError("archive_directory must be a real directory")

    destination = Path(output_directory).expanduser().resolve()
    destination.mkdir(parents=True, exist_ok=False)
    ingested = _as_utc(ingested_at_utc, name="ingested_at_utc")
    bundle_names: list[str] = []
    archive_records: list[dict[str, object]] = []
    total_rows = 0

    for year in years:
        observations, metadata = parse_histdata_context_archive(
            _find_archive(raw, year, symbol),
            year=year,
            source=source,
            symbol=symbol,
            ingested_at_utc=ingested,
        )
        data_name = f"{source.source_id}-{year}.parquet"
        manifest_name = f"{source.source_id}-{year}.manifest.json"
        data_path = destination / data_name
        write_parquet_atomic(data_path, observations)
        start = observations["observed_at_utc"].min()
        end = observations["observed_at_utc"].max() + pd.Timedelta(1, unit="ns")
        write_json_atomic(
            destination / manifest_name,
            {
                "schema_version": 1,
                "source_id": source.source_id,
                "availability_basis": source.availability_basis,
                "observation_start_utc": start.isoformat(),
                "observation_end_utc": end.isoformat(),
                "row_count": len(observations),
                "format": "parquet",
                "file": data_name,
                "sha256": sha256_file(data_path),
            },
        )
        bundle_names.append(manifest_name)
        archive_records.append(metadata)
        total_rows += len(observations)

    set_path = destination / f"{source.source_id}.bundle-set.json"
    write_json_atomic(
        set_path,
        {
            "schema_version": 1,
            "kind": "bundle_set",
            "source_id": source.source_id,
            "availability_basis": source.availability_basis,
            "bundles": bundle_names,
        },
    )
    source_archives = destination / "source_archives.json"
    write_json_atomic(
        source_archives,
        {
            "schema_version": 1,
            "source_id": source.source_id,
            "source_symbol": symbol,
            "availability_basis": source.availability_basis,
            "availability_is_historical_evidence": False,
            "years": list(years),
            "archives": archive_records,
        },
    )
    return {
        "source_id": source.source_id,
        "source_symbol": symbol,
        "availability_basis": source.availability_basis,
        "strict_pit": False,
        "years": list(years),
        "rows": total_rows,
        "bundle_set": str(set_path),
        "bundle_set_sha256": sha256_file(set_path),
        "source_archives": str(source_archives),
        "source_archives_sha256": sha256_file(source_archives),
    }


__all__ = [
    "HISTDATA_CONTEXT_SOURCE_OFFSET",
    "ContextHistDataError",
    "import_histdata_context_archives",
    "parse_histdata_context_archive",
]
